...
router.post("/update/:id", upload, async (req, res, next) => {
  ...
  await Toy.findOneAndUpdate({ _id: id }, upload, function(err, toy) {
    if (err) {
      ...
      if (toy.toyName !== req.body.toyName) {
        toy.toyName = req.body.toyName;
      }
      if (toy.description !== req.body.description) {
        toy.description = req.body.description;
      }
      if (toy.age !== req.body.age) {
        toy.age = req.body.age;
      }
      if (toy.location !== req.body.location) {
        toy.location = req.body.location;
      }
      if (toy.category !== req.body.category) {
